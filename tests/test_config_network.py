...
def test_proxy_hosts_list_default_off_parses_direct_sites():
    cfg = Config(direct_sites=" javdb.com , missav.ws ")

    assert cfg.proxy_hosts_list() == ["javdb.com", "missav.ws"]


def test_proxy_hosts_list_route_all_returns_wildcard():
    cfg = Config(direct_sites="javdb.com", proxy_route_all=True)
