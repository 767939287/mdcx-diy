...
- Instructions:
  ...
  - **libredmm 全站数据源与归纳集成（2026-08-30 完成并集成，提交 e0c98b7）**：`/movies?page=N` 每页 30 条按字母序、**列表页缩略图 src 直接含完整 DMM cid URL**（`<a href="/movies/番号"><img src="...cid...ps.jpg"/>` 同元素），翻页即批量取「番号↔cid」；番号与 cid 字母部分不要求一致（厂牌自定义编号，如 000_339↔n_630aps176）。全量抓取 23472 页 588,532 对（脚本 `crawl_libredmm_full.py` 与 161 批产物在 /tmp/opencode/），归纳三轮迭代：① combo 结构（prefix×cid_series 全枚举解前缀截断歧义，VRKM→v+rkm）；② 数字位数集合（存在 pad6 如 `5642hodv010006`，硬编码 3/5 位不够）；③ other 阈值放宽 50%（重编号条目落白名单），最终 9627 系列 96.5% 直构命中。**安全过滤两轮演进（教训链）**：v1 发现 libredmm 同名番号被 mono 老厂牌占据（SSIS-001 记录的是老厂牌 `k9ssis001`），整体弃用静态表覆盖系列与 mono 数据——但 GVG-564 真实 cid 就是 mono 3 位 `13gvg564`，v1 把老片系列的真实数据也丢了（日亚重裁决时 517 行裁判图 miss 暴露此缺口）；**v2 修正为 append 模式**：静态表覆盖系列的路由候选不弃用、追加在静态表候选之后兜底（静态表保序首位，高频系列顺序零污染），未覆盖系列保持 first 一步直达，combo 按 digital 路径/5 位优先排序。当前 v2.1 种子：9267 系列（110 append）+ 10530 白名单，1.8MB，测试 45 项锁定。**外部归纳数据接入生产前必须先在高频样本上做候选顺序回归——覆盖率数字会掩盖顺序污染；但"防污染"也不能矫枉过正弃掉真实增量数据，安全过滤要按"顺序影响"分级（append 兜底）而非二元弃用**。
  - **日亚数据库双副本与清洗工作台（2026-08-31 梳理）**：`userdata/amazon_asin_database.xlsx` 是运行库（41164 行，历史裁决已落库，主表 0 冲突 + 「待修正」sheet 929 行工作台）；`resources/userdata/` 是出厂库（9700 行，193 组冲突未清，随包分发）。**更新出厂库须用户明确确认**（2026-08-31 用户指示），不可自作主张用运行库全量替换。重裁决脚本 `/tmp/opencode/rejudge_pending.py`（checkpoint 断点续传，逐行落盘），裁决结果 `/tmp/opencode/rejudge_result.json`，行动清单 `/tmp/opencode/rejudge_action_plan.json`。
  - **待修正 929 行重裁决最终结论（2026-08-31 完成）**：match 327 / mismatch 568 / no_ref 22 / no_dmm 6；**全部 929 行无一需要落库**。match 327 细分四处置：① 缩位冗余 105（ABF-34/MFYD-44 类——同一番号缩位与规范写法两条记录互认配对，主表已有规范形态，救回会制造重复；**裁决前必须做番号规范化预检，写法不规范会制造"自己和自己冲突"的假案**，用户两次抓样例挤掉了 match 的水分）；② 番号重复 117（番号在主表已挂其他 ASIN，一码多商品正常形态）；③ ASIN 占用 2（救回会制造新冲突）；④ **DMM 高清覆盖 103——虽然配对为真（0.85-0.99 高分），但生产预筛 `_should_skip_amazon_for_existing_poster` 下爬虫拿到宽≥700 的 DMM 高清图即跳过 Amazon 搜索，实测 103 个番号全部能取到 DMM 高清图（1032x1467/800x539 级），日亚流程永远不会走到，救回只是死数据**。**评估日亚库存价值的方法论（用户点拨）：先问"这个番号生产上会不会走到日亚那一步"——DMM 能给高清图的番号，其日亚记录无运行时价值**。注意探测 DMM 高清图须按生产标准（宽≥700）过滤缩略图：awsimgsrc 同 URL 会返回 147x200 缩略图形态（约 10-19KB，恰好过 4KB 占位图阈值），取"第一个下载成功"的图会误判，必须跳过缩略图继续探测。
  - **裁决图选取缺陷（2026-08-31 实测踩坑）**：裁判图**不能取第一个下载成功的就停**——同番号可能存在 digital 再版（ABF-008 的 `436abf00008`）与 mono 原版（`118abf008`）两种封面，须遍历全部直构候选取最高相似度；日亚商品挂哪版由最高分决定。同系列多集误挂同一 ASIN 的清理（GVG-019/032/045 同挂 B07NYS8SSP）不是错杀——系列片名相同但各集封面不同，相似度 0.63-0.66 是不同集的真实差距，winner 集必然分高。
  - **长任务事故复盘（2026-08-31）**：重裁决首轮后台终端 1 小时超时上限被杀（跑到 210/929 死亡，日志 0 字节——stdout 全缓冲未落盘，wrapper+tail 管道教训重演）；checkpoint 逐行落盘保住全部成果无损。修复：`-u` 强制无缓冲 + wrapper 45 分钟自重启续跑（libredmm 抓取验证过的模式）。**后台长任务一律 checkpoint + wrapper 自重启，判进度看落盘文件而非终端日志**。
