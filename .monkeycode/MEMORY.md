...
- Instructions:
  ...
  - 五站爬虫已删除（2026-08-25 用户决定）：cnmdb（域名劫持博彩页）、hdouban（DNS 失效）、mdtv（默认域名 302 广告停放）、love6、kin8（站点改版故障）。若用户日后提供新域名，从 git 历史恢复对应 mdcx/crawlers/*.py 并重建 Website 枚举/注册/默认源即可。
  - 7mmtv 现状（2026-08-25）：7mmtv.sx 新上 CF「Just a moment」JS 挑战，纯 curl_cffi 指纹全拦；7mmtv.tv 在 devbox SSL 断连。过挑战依赖 TRAWL/FlareSolverr 外部 CF Bypass 服务（AsyncWebClient 刮削路径与网络检测均已自动接入）；devbox 出口对该站时段性整体断连，验证失败先换时段重试。
