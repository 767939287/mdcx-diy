...
- Instructions:
  ...
  - **改完代码必须先跑 quick-check 再跑完整 check**（2026-08-23 用户强调）：完整 check 含 ruff format 检查，跳过 quick-check 直接跑完整 check 会在 2 分钟后才发现格式问题、白等。流程：每次改动 → quick-check（抓格式/lint/类型）→ 通过后再完整 check。
