...
[排查过程中发现的问题必须主动报告]
- Date: 2026-08-21
- Context: 全面检查 UI 文字时，subagent 在排查过程中发现了 7 处问题，但最终汇总报告只提了部分，遗漏了在探索阶段发现的其他潜在问题（如 changelog 中历史条目的命名不一致、测试文件中的 mock 数据引用等），用户发现后要求全量报告
- Category: 工作流与协作
- Instructions:
  - 排查/探索过程中发现的**所有**问题（包括已修复的、暂不修复的、仅作记录的）都必须在最终报告中列出，不能因"已修复就不再提"或"太小不值得提"而遗漏。
  - 报告格式：按严重程度排序，每条包含文件路径+行号、当前内容、问题描述、建议。
  - 即使某个问题最终判定不需要修改（如代码模块名 vs site value 的区分），也要列出并说明为什么不改，让用户自行决定。
  - 主动提出建议：发现问题后不只是罗列，还要给出修复方案、优先级建议和潜在影响评估。
