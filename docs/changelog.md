...
## v2.0.5 (2026-08-06) 当前版本

### 功能

- **移除「从 AVdb 同步」入口**：AVdb（Jav-Actors-Mapping）数据经全面排查确认质量差——错误 id 映射（`平山加奈`→Christa Allen、`森千里`→歌手森高千里）、混入大量非 AV 人物（好莱坞影星/声优/偶像）、重复行、生日误填。用户不再使用该数据源，移除工具页「从 AVdb 同步」按钮及配套 UI（数据源下拉、地址输入、选择文件、说明文字）、信号连接与处理函数。保留 `sync_from_avdb` 底层函数与测试，供脚本/未来复用
- **移除失效的「校验 tmdbid 与名字匹配」复选框**：该复选框是「从 AVdb 同步」的附属选项，同步入口移除后成为孤儿控件（无任何代码读取），勾选与否均无影响，已移除
- **使用说明 tab 更新**：第九节「工具页面」演员库维护描述改为当前 7 个操作（补全中文名/补全 LibreDMM 链接/同步别名/打开数据库/剔除男演员/校验 tmdbid 有效性/更新 nfo tmdbid）；修正第十节「素人」重复条目（误残留的网站列表行删除）
- **演员库维护工具联网健壮性改进**：
  - **独立停止按钮**：工具页新增「停止当前维护任务」按钮，与主界面刮削停止相互独立，一键请求停止当前维护任务；4 个联网维护工具在滑动窗口每轮响应停止，取消 pending 任务并保存已处理部分，日志输出已处理进度，可再次运行继续剩余
  - **校验 tmdbid 由全量并发改为滑动窗口**：`verify_tmdb_ids` 原先 `asyncio.gather` 一次性创建全部并发任务，现改为滑动窗口（默认并发 5），每 10% 输出进度日志，停止响应更及时，内存占用更低
  - **限量分片断点续传**：补全中文名/补全 LibreDMM 链接/同步别名/剔除男演员/校验 tmdbid 统一支持 `limit` 限量参数（默认 5000），配合工具天然幂等性（如补全只处理缺中文名的行），可多次运行逐片处理 2 万+ 行本地库，避免单次长时间联网占用
- **出厂演员库去重清洗**：新增 `scripts/merge_actor_db_duplicates.py` 筛查并修复「无 tmdbid 行」与「有 tmdbid 行」的重复（同一演员多个艺名被拆成多行）。确证标准为无 id 行的别名精确包含有 id 行主名（经全半角/繁简/大小写规范化）。本次合并 17 个有 id 行（别名并集 + 空缺字段补全）、删除 18 个无 id 行，含「みちる」(id=5557511) 三行同人合并。执行前自动备份 `.bak.xlsx`
...
- **补全别名只处理空别名行**：`run_actor_db_xlsx('sync_aliases')` 原先全量扫描 5484 个有 id 行（约 50 分钟），现仅处理别名列为空的条目（24 条），效率大幅提升；按钮「同步别名」更名「补全别名」、说明改为「仅补全缺别名的条目（已有别名的行跳过）」，消除"全量同步最新别名"的歧义
...
- **内置男优名单扩充与修正**：新增 `scripts/add_male_actors_from_wiki.py` 从 AVWikiDB「AV男優一覧」页提取男优名（清洗括号别名标注、过滤噪声标签/导航词），与现有名单求差集追加。本次从 4 页补充 74 个男优（625 → 699）；随后核查发现「水川潤」「田村歩」实为**女优**（TMDB gender=1）误收，已从名单移除（→ 697），避免剔除男演员误删女优
- **剔除男演员走断点续传**：`clean_male_actors` 原先 `limit` 每次取前 N 个有 id 行、重跑重复校验同一批，无法覆盖全量。现新增 `.gender_checked.json` 断点文件——已校验 gender 的 tmdbid 记录在案，重跑自动跳过，支持分片续跑全量
- **出厂库剔除男演员清洗**：隔离副本真实联网（`api.tmdb.org`）全量校验 5484 个 tmdbid 的 gender + 内置名单命中，删除 4 个男优（ムール貝瀬、伊佐美、稻荷卯月、川村慎一），删除前备份到「男优备份」sheet。出厂库 20191 → 20187 行，无重复，女优「水川潤」等未误删
- **男优备份全面核查**：全量核查「男优备份」1196 行——有 id 的 1192 个中 gender=1（女）为 0，名字女性化的（中山佳子/秋山智美/浅野ゆう子/蜜香/辰美 等）经 TMDB 核实全部为 gender=2 真男优；12 个 gender=0 均为 adult=True 成人演员（含加藤鷹/鮫島）。唯一存疑「望月未来」（gender=0、adult=False、作品仅 1 部）按「宁漏勿误删」原则从男优名单移除（→696），备份保留作记录
- **出厂库非演员清洗**：新增 `scripts/clean_actor_db_non_actors.py`，删除三类混入行共 457 个（出厂库 20187 → 19730）——
  - **导演/幕后 439 个**：TMDB `known_for_department` ≠ Acting 的 AV 从业者（Directing 273、Visual Effects 38、Writing 32、Production 24 等）
  - **描述词/占位符 16 个**：本妻/女優不明/愛嬌抜群のおんな 等非人名
  - **非 AV 主流人物 2 个**：埃里克·坎通纳（足球明星）、艾琳娜·拉尼娜（主流演员）
  修复 delete_rows 批量删除导致的末尾空行残留与超时问题（合并区间删除 + 清理尾部空行）
- **清理漏网男优 23 个（gender=0 识别盲区）**：核查发现一批 TMDB `gender=0`（未标性别）的男优混入演员库——男性名 + adult=True + 作品佐证（含知名男优阿部次郎，id 与井草雄二/南大介等连续，疑同一数据源批量灌入）。此前男优识别仅依赖名单命中 + TMDB gender==2，对 gender=0 男优完全失效。已删除并从男优名单补录 23 个（696 → 719），出厂库 19730 → 19707 行
- **清理占位/冗余行 283 个**：删除「前 4 列（日文/中文/繁体/别名）完全同值 + 后 5 列全空」的无信息量行（如安田みう/阿川奈那子），出厂库 19707 → 19424 行。注意 openpyxl `read_only` 模式会丢失部分行的链接数据导致误判，占位行判定须用非只读模式
