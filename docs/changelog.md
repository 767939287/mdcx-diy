...
- **#144 最大化后封面/缩略图不随框放大、简介/标签仍只显示单行**：①出图流程残留设计尺寸硬编码（`resize(156×220 / 328×220)`），切换封面时会把已放大的框砸回原尺寸，且窗口缩放只改框、不重渲染图片；现缓存原图并在框几何变化时按等比 + 平滑重缩放，切封面/缩放/还原三态一致，出图不再改写框几何。②简介/标签行高此前恒为设计 40px，长文本被压成一行；现随窗口高度每行最多增高 60px（默认窗口与最小化仍为设计 40px，双向幂等），下划线贴行底、其下日期/导演/制作等各行整体让位，拉高即可多读几行。③最小化小窗为验收边界，布局保持设计稿不变
