...
- **#134 演员管理器列表优化**：在「详情」与「标签」之间新增「出生日期」「出生地」列（取 Emby `PremiereDate` 前 10 位与 `ProductionLocations`，未设置生日按空值），并把「详情」「标签」改为自适应拉伸，填满宽窗口右侧空白

- **#131 剧照下载失败后回退下一剧照网站**：按剧照字段优先级收集各来源地址，逐来源整组下载，成功即原子替换、失败换下一来源、全部失败沿用本地旧文件（原实现只试单一来源）

- **#125 维基百科/维基数据刮削触发速率限制（429）**：对 wikidata/wikipedia 域名单独限速 1 req/s（原通用 8 req/s 远超其限额），并把 wiki 请求 UA 改为带项目地址的可识别 UA，归入 200 请求/分档

- **#124 最大化后封面/缩略图黑框盖住下方信息区字段**：封面区按窗口宽度横向放大后，下方简介/标签/日期等字段与勾选框仍停在设计坐标被覆盖出重影。现信息区右移到「缩略图框右缘 + 15px」、尺寸文字与勾选框跟随封面框底边（设计宽度下零变化）
