...
- **议题 #132 托盘隐藏主窗后操作演员管理器仍会弹出主窗**：主窗通过托盘图标隐藏（或「关闭到托盘」「最小化到托盘」）后，对 Emby 演员管理器做任何操作都会触发应用激活事件（ApplicationActivate），而主窗的 `eventFilter` 会对「非最小化的隐藏态」调用 `show()` 把主窗拉出前台——所以用户只有先最小化、再隐藏才不弹（最小化态被单独放行）。现移除该自动 `show()`：隐藏是用户主动行为，恢复显示只由托盘图标/托盘菜单负责；最小化态仍由 Qt 自行保持。新增回归测试覆盖「非最小化隐藏态不被 ApplicationActivate 弹出」「托盘隐藏后操作演员管理器主窗保持隐藏」「托盘图标点击仍能正常恢复显示」

- **议题 #134 演员管理器列表新增出生日期/出生地并加宽详情、标签列**：演员管理器主表格原先只有状态/姓名/头像/简介/详情/标签/影片数 7 列，「详情」「标签」固定宽度（350/200），在 1920 宽窗口下右侧留下大片空白。现于「详情」与「标签」之间新增「出生日期」「出生地」两列（分别取 Emby 的 PremiereDate 前 10 位与 ProductionLocations，未设置生日时按空值展示），并把「详情」「标签」改为自适应拉伸，自动填满窗口右侧空间；新增回归测试锁定列顺序与拉伸模式

- **议题 #131 剧照下载失败后不尝试下一个剧照网站**：剧照能刮到 URL，但目标站已删除图片导致下载 404 时，程序只按单一来源处理、失败后不再尝试其它剧照网站（常见于 javdb 有、avbase 没有，或反之）。现按剧照字段优先级收集各来源的剧照列表，下载时按顺序尝试，任一来源整组下载成功即原子替换到 `extrafanart` 目录并结束；整组失败则清理该来源的临时目录、回退下一来源；全部失败仍沿用本地旧文件（无旧文件计为下载失败）。新增回归测试覆盖「主来源失败回退次来源」「全部失败保留旧文件」「重复 URL 去重」「候选按优先级收集」

- **议题 #125 维基百科/维基数据刮削触发速率限制（429）**：批量补全 Emby 演员信息时，程序对 Wikidata/Wikipedia 的请求沿用了通用 8 req/s 的域名限速、并携带随机浏览器 User-Agent，远超维基媒体的限额（未识别 10 请求/分、仅合规 UA 200 请求/分），被限流后重试又会进一步放大压力。现对 wikidata.org / wikipedia.org 域名单独设 1 req/s 慢速限速（重试同样受限），并把 wiki 请求的 User-Agent 改为带项目地址的可识别 UA，归入 200 请求/分档；新增回归测试锁定「wiki 域名走慢速限速、请求带可识别 UA」

- **议题 #124 软件界面最大化后封面/缩略图黑框盖住下方信息区字段**：主界面最大化时，封面图与缩略图展示框按窗口宽度横向等比放大（160×220 设计框在 1920 宽窗口下放大到约 322 高、缩略图右缘伸到约 1208），而下方的简介/标签/日期/导演/制作/厂牌/系列/发行等字段、以及「保存封面/保存缩略图」勾选框仍停在设计坐标（x≈30~490、y≈380），于是被放大的黑框横向盖住、纵向压叠，出现文字被黑框遮挡的重影。现把整个信息区右移到「缩略图框放大后右缘 + 15px」处、内部各列间距保持设计值并随窗口宽度压缩，尺寸文字与勾选框的纵向位置改为跟随封面框底边；设计宽度（820）下零变化，缩放反复触发不累积漂移。新增几何回归测试锁定「信息区字段左缘不得落入缩略图黑框内、尺寸行顶须等于封面底」
