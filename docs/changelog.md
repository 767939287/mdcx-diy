...
- **议题 #106-① 带 DMM `z` 尾缀的番号在 javdb/javbus 搜不到（IBW 系列等）**：DMM 图床部分番号带 `z` 尾缀（如 `IBW-786z`，站内收录为 `IBW-786`），刮削直接拿带尾缀的番号搜索，javdb/javdb_app/javdb_api 一律「未收录该番号」、javbus 直链与搜索兜底都匹配不上。现放宽搜索与命中：番号带 `z` 尾缀时，javdb 系搜索候选与 javbus 直链/搜索匹配一并附带去掉尾缀的基础番号（原番号优先、保序去重），命名与落库仍保留 `z` 原样
