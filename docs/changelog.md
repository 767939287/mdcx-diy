...
- **议题 #115 刮削探测超时 15s→30s，重试失败项递进 45s/60s**：用户反馈慢站/慢网/过 CF 时 15s 探测超时仍不够，建议放宽到 30-60s。现首轮全量检测的刮削探测超时提到 30s；点「重试失败项」时按轮次递进——第 1 次重试 45s、第 2 次及以后 60s（封顶），检测页同步提示本轮超时值。递增只作用于手动重试的少数失败项，首轮耗时不受影响；多次重试仍超时基本可判定该站当前网络下确实刮不动（真正连不上的站点会在更早的基础连通性检测阶段就失败）
