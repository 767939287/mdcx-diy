...
- **无 id 演员重查补 id**：对 291 个「被清除 id + 仅名字」的无 id 演员按名字重新搜索 TMDB——发现大量旧 id 因 404 被清除的演员在 TMDB 上有重建后的正确档案（用户提供 `三佳詩` 正确 id 5882313 触发排查）。修复繁简匹配后补回 20 个正确 id（`三佳詩`→5882313、`楓カレン`→2638477、`柚希つばさ`→2411598、`森千里`→6220535 等），剔除 13 个误补回的非 AV（`未来`→舞台剧演员、`しずか`→宇野志津香、`AI`→偶像 等），合并 2 组补 id 引入的重复行（`純名きりん`=加納瑞穂、`晶エリー`=Eri Akira）。出厂库 20211 → 20209 行、有 tid 5252 → 5270
