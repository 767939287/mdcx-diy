...
- **左侧导航 Emby 演员管理点击无反应**：`btn_settings`（设置）按钮在「移除冗余测试连接」时被误删但信号连接残留，对话框构造抛 AttributeError（异常被吞）；补回按钮
- **左侧导航新按钮样式缺失**：Emby演员管理/NFO库管理 未纳入导航按钮样式选择器，未选中也显示默认背景框；补齐统一样式与 hover（深/浅主题）
- **工具页演员库维护 group 高度不足**：内容最低点超出 group 固定高度，底部 JavDB 中文名按钮与说明文字被裁剪；group 加高 60px 并整体下移后续 group 保持 20px 间距
- **演员库维护 group 文字被裁 + 字体过小**：说明文字 11px/部分 9px 过小，且 5 处说明 label 高度不足被上下裁剪（含「起始行」与「JavDB 中文名」间说明需 42px 仅 20px）；统一调至 12px、补齐 label 高度、连锁调整相邻控件与 group 高度
- **网络页「网络设置」group 文字重影/显示不全**：grid 内长说明 label 垂直策略 Fixed 锁 30px（需 70px 显示 5 行文字）导致溢出被遮挡；改策略 + minimumSize 高度 70；同时发现并修复 grid row 6 两组控件（超时滑块与 TRAWL 输入）重复占位重叠（TRAWL 移入空闲 row 4）；grid widget 加高 50px、group 加高 30px、后续 group 下移保持间距
- **类型刮削网站 group 批量操作框错位**：`groupBox_nfo_lib_batch` 的 grid item 缺 row/column 定位，被 Qt 放到 (0,0) 退化为 100×30 与「字段优先级」说明文字重叠；补 row16 定位并加高 group 与滚动区
- **批量操作放错页面**：NFO 批量操作面板被放在「类型刮削网站」设置里，与操作对象（NFO 列表）分离，用户找不到列表；已迁移到 NFO 库管理页左栏列表下方（含说明文字），并回滚类型刮削网站 group 为容纳它所做的加高
- **类型刮削网站说明文字右侧空白**：国产/动漫里番/Mywife 三处说明 label 仅占 col1，右侧编辑按钮列空白；改 colspan 3 充分利用宽度（535px），放宽国产说明 max-height
- **Bypass 落地白名单缺少常驻说明**：该配置只有 tooltip/placeholder（hover 才可见），用户不理解用途；新增常驻说明文字（校验 Bypass 落地域名防劫持，留空不校验，一般无需配置）并连锁加高网络设置布局
- **演员名白名单说明不清**：补充用途说明（限制 NFO 标签中出现哪些演员名，留空=全部演员写入）
- **UI 与文档一致性检查**：按「功能改动同步 UI/文档」规范全量核对——网站数量 48 全一致；补 missav.live 到 UI 说明页与 USER_GUIDE/FEATURES/DEVELOPMENT 代理列表（CONFIGURATION 已有）；DMM 兜底描述统一更新为「官方图源兜底（DMM + MGStage）」到 UI 说明页/USER_GUIDE/CONFIGURATION；主窗口「所有可用网站」弹窗有码列表补 thejavdb_api
- **下载页「压缩」tooltip 不清**：仅写"压缩图片"，补全实际行为——压缩刮削后所有图片（封面/海报/缩略图/剧照），长边>1100px 缩放到 1100，JPEG/WEBP 质量 80，PNG 无损
- **批量操作缺说明文字**：类型刮削网站 group 底部批量操作面板无任何说明，补充用法（先选中多条 NFO → 填内容点按钮 → 批量保存）
