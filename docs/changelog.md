...
- **MDCx 单实例锁（检测网络/外部 CF 适配层等路径曾二次 exec main.py 致双开主界面）**：入口原本无跨进程保护，检测网络过程中命中外部 CF 适配层（`trawl_adapter` 用 `sys.executable` 起 uvicorn 子进程，PyInstaller 下子进程显示名与主程序一致）等场景下，任务管理器会出现第二个「MDCx 进程」、屏幕同时冒出两个一样的主界面。现用 `QLockFile`（Qt 原生跨进程文件锁，PyInstaller/源码两种模式均可靠）在 `main.py` 早期加单实例锁：首个实例 `tryLock` 成功并持锁至进程退出；被二次启动的实例 `tryLock` 立即失败、不再创建第二个窗口直接返回。锁文件落系统 temp 目录，进程退出自动释放，不影响正常冷启动
