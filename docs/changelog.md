...
- **议题 #117 信息管理页小窗时表单被裁、保存按钮看不见**：窗口未最大化（小窗）时，右侧表单列被垂直滚动条挤掉十几像素宽度，输入框左侧圆角正常、右侧被裁成平口；同时「保存当前 NFO」按钮被推到视口之外，要下拉才看得到。现小窗时自动压缩「简介/标签」两个多行框的高度（60px → 最低 40px）换取整表免滚动显示——垂直滚动条消失、输入框右端恢复圆角、保存按钮直接可见；空间充足（含最大化）时完全保持现有布局不变

- **议题 #115/#118 刮削探测超时 15s→30s，并在同一轮内自动递进重试到 45s/60s**：用户反馈慢站/慢网/过 CF 时 15s 探测超时仍不够（#115 建议放宽到 30-60s，#118 进一步要求首轮就自动重试）。现单站刮削探测按 30s → 45s → 60s 最多自动探 3 次，任一次通过立即定论；三次都没过则给出「站点可达但刮削探测 3 次均超时（30s/45s/60s），判定该站刮削探测无效」的终局说明。只有超时/搜索页请求失败/探测异常这类**瞬时性**结果才重试，「测试番号未被该站点收录」「被 Cloudflare 拦截」首次即定论（重试必然同样结果，白等无益）。重试期间打印「第 N/3 次刮削探测（超时上限 Xs）」，检测页头部同步标注该策略；点「停止」在下一次探测前生效，不必等满当前阶梯。「重试失败项」回归纯重测（同样走这条阶梯），用于改完代理/CF 配置后再验一次；结果汇总新增「刮削探测多次超时」根因分组，把这类站与「未收录」「连不上」区分开并给出换节点/稍后再测的建议。代价是全部站点都超时的极端情况下首轮最长约 9 分钟，实际只有真正超时的站才付这个等待

- **议题 #114 网络设置新增「直连白名单」（保留原「使用代理网站」列表）**：用户反映「使用代理网站」列表要维护的站点太多、默认清单常缺新站，增减时容易漏改错改。现设置页网络区在原「使用代理网站」下方新增「直连白名单」输入框（逗号分隔，如 `google.com,github.com`），优先级高于原列表——命中白名单的站点强制直连、跳过代理，其余站点仍按原「使用代理网站」列表决定是否走代理；白名单默认留空，旧配置行为完全不变，按需填入想直连的站点即可

- **议题 #113 统计栏新增「原始条目数 / 唯一名字数」切换开关**：用户反映 Emby 演员总数与 MDCx 统计不一致，无法分辨是去重导致还是真的数据差异。现统计栏顶部增加「计数方式」下拉框，选项为「原始条目数」（Emby `/Persons` 返回列表长度，预去重）和「唯一名字数」（当前 actors 列表去重后的人数），切换即时刷新，同步后自动刷新也会正确回写 raw_count 并刷新显示

- **有码封面设置裁剪后仍输出横版（MGStage 等「有码但源图为横版直出」站点）**：勾选「有码封面不裁剪」之外的裁剪设置后，MGStage 这类有码站点直下的横版封面（如 800×539）未被右裁成竖版海报，而普通有码站走 thumb 右裁剪却正常。根因三处：①未勾选「有码选优」时，该站爬虫标记 `image_download=True`，直下横版海报成功后即返回、跳过有码右裁剪；②勾选「有码选优」时，选优按尺寸比较竖版候选，但 crop 尺寸探测失败（本地源图未落盘）或候选尺寸全未知时，竖版过滤被跳过、横版候选漏出直下；③候选全部失败后 MGStage 官方图源直构兜底下载横版图，成功后直接落盘、同样跳过裁剪。现修复：非 VR 有码在任一直下成品为横版且本地有 thumb/fanart 时，按有码右裁策略本地重裁并替换海报（普通直下与 MGStage 兜底两路均接入）；选优路径竖版过滤改为无条件执行、已知/未知 crop 尺寸下横版候选一律不直出，尺寸全未知时退回 thumb 右裁剪；crop 尺寸计算对 fanart/thumb 逐个校验实际落盘再取，避免并发下载时读到未落盘路径。无码/素人/VR/本地无图源不受影响
- **议题 #100-① 再次刮削失败文件被重复计数**：用户实测上次失败 11 个、再次刮削变 22 个——扫描到的失败文件（重试未超限不会被跳过）与断点缓存自动「恢复」的同一批文件被直接 `extend` 追加，同一文件入队两次。现恢复去重（重复项丢弃），标签里只剩真正不在本次列表中的失败文件被追回
- **议题 #100-③ 「图片已被网站删除」日志不标来源站点**：图片校验失败的红色日志此前只附 URL，用户面对多项图源排查时无法定位是哪个数据站。现日志行统一带 `[host]` 标签（如 `[awsimgsrc.dmm.co.jp]`），一眼锁定图床来源
- **议题 #100-② 连续失败 3 次不再自动重刮的提示不明确**：「断点续刮跳过 N 个连续失败超过 3 次的文件」原提示仅一句"强制重刮可重试"，用户找不到出口、只能删配置重置。现提示改为直给入口：「软件日志页点『失败』展开失败列表，点『一键重新刮削当前失败文件』」
- **议题 #101 字段来源表「已失败」不区分失败类因**：用户看到 `avbase/thejavdb_api/javdb_app (已失败, 后续字段将跳过该站)` 全红，误判为逻辑 bug（实际是这些站在其网络下请求级失败——走代理/连不上）。现失败标记附类因：`已失败，原因: 请求超时 / 被站点拦截 / 请求异常`，一眼分辨"站点连不上"与"站点没收录"
- **映射云盘原子写入静默丢失（刮削日志显示「Nfo done!」但盘上无 nfo 文件）**：用户实测 v2.0.9 批刮 60 部（落盘在 115 云盘映射的 `Z:`），日志全部显示 `🍀 Nfo done! (new)`，实际目录里一个 .nfo 都没有——映射云盘驱动上 `os.replace` 会返回成功但目标文件实际未落（静默吞写），旧的原子写路径毫无察觉、假报成功。现文本原子写（NFO / 演员库 / Emby 演员等共用 `write_file_atomic(_async)`）在 `os.replace` 后强制校验目标存在；校验不过自动退回直接写入兜底并记录告警，直写仍不落则直接报错——文件要么真落盘、要么计入失败列表，绝不再假报成功
- **议题 #98-1 调试日志三个开关形同虚设**：「显示刮削过程信息 / 显示字段来源信息 / 显示字段内容信息」三个开关此前互相依赖——日志读取端只判「刮削过程信息」一个开关，关闭它时把已按各自开关写入的字段来源块、字段内容块整段丢弃，另两个开关怎么设都无效。现新增独立的过程明细日志通道：爬虫请求、图片下载、TMDB 查询、翻译过程等明细只在「显示刮削过程信息」开启时输出；字段来源/字段内容块真正只受各自开关控制；文件/番号、站点耗时、Data/Folder/Movie 完成、失败原因等关键节点行恒定可见。三个开关任意组合独立生效
- **议题 #98-3 「显示字段来源信息」输出精简**：此前 `show_from_log` 开时展开「📒 字段来源」逐字段排障块（`📌 字段` / 优先级设置 / 每站 未找到·已失败·实际值），普通用户看来源时排障明细淹没站点摘要。现逐字段排障块归过程明细通道——`show_from_log` 开时输出保持 `[website] 站点名(语言)(耗时)` 摘要精简形态，逐字段排障明细仅在「显示刮削过程信息」开启时输出，完整排查按文档说明三开关同开
- **议题 #98-2 手动停止后「继续刮削剩余任务」丢失任务**：停止后再续刮会丢任务、跳过已完成或误清断点记录，五处同源修复——①剩余任务列表改原子写入（先写临时文件再整体替换），读取端不可能再读到空文件或写了一半的文件；②后台定时保存与任务完成存在竞态：保存期间又有任务完成时，旧保存进程会误清"待保存"标志导致最新列表永不落盘，现落盘后按快照比对才清标志；③点「停止」与刮削协程退出时立即强制落盘一次最新剩余任务，不再只依赖 1.5 秒定时器；④续刮时不再拿「剩余任务子集」去清理整个断点缓存——旧实现会把子集外其余文件的断点记录误判"源文件已不存在"而清光；⑤续刮启动改传列表快照，不与新刮削共享同一个可变列表
- **议题 #98-3 大批量刮削后出现 `Task was destroyed but it is pending!`**：流式响应提前放弃时程序只同步发起中断便立即返回，curl_cffi 内部的接收任务此刻尚未退场，协程随后结束就触发该警告（正常完成与手动停止后的清理阶段均会偶发）。现中断后给内部任务 0.5 秒退场宽限、超时才取消并消费异常，保证返回时任务已终结；影片处理结束时对共享图片任务同样改为等待收尾完成再退出。既有的「中断立即返回、不拉满剩余响应体」性能行为保持不变
- **议题 #96 网盘/映射盘目录创建失败被误报为「权限不足」**：刮削归档在网盘/映射盘（如 115 云盘 `Z:`）时，云盘对单个文件夹名的长度/字符限制比本地 NTFS 更严，目录名超限时系统返回 `WinError 123`（文件名/目录名/卷标语法不正确）。程序此前把这类失败笼统报成「创建文件夹失败！请检查是否有写入权限」、失败汇总归为「权限不足」并提示「以管理员身份运行」，与实际原因（目标盘对文件夹名的限制）不符，误导排查。现识别 WinError 123 后改为提示「目标盘可能限制文件夹名的长度或字符」，失败汇总归类为「目录名无效或过长」，并建议「调小目录名最大长度或先刮到本地盘再同步」
- **Lulubar（撸撸吧）网络检测误报「测试番号 SSNI-647 未被收录」**：网络检测对该站沿用全局探针番号 SSNI-647，而 Lulubar 并未收录该番号，于是站点明明连通、刮削正常也给出「未被该站点收录」的警告。现改用该站确认收录的探针番号 IPZZ-547；并修正搜索结果番号匹配的大小写处理（此前只把输入番号转小写、站内标题原样比较，标题为大写番号前缀时一律匹配失败）
- **议题 #98 追加反馈：大批量刮削时连续报 curl_cffi `cleanup TypeError`**：实机日志反复出现 `Exception in callback AsyncSession._request_once.<locals>.cleanup()`（`curl_multi_remove_handle` 收到已失效的 curl 句柄）。现关闭流式响应改为先中止传输、再交由 curl_cffi 自身归还句柄，异常根除；提前放弃的图片尺寸探测仍立即返回、不拉满剩余响应体
- **议题 #98 追加反馈：停止/收尾后偶发「网络客户端等待空闲超过 300 秒仍未空闲（残留租约 1）」**：点「停止」会无差别取消刮削协程，取消恰好落在收尾动作的执行窗口时（释放连接租约的 await 被打断），租约计数永不归零——之后保存/切换配置时旧网络栈要空等 300 秒才强制关闭，且网络与 LLM 客户端各残留 1 个。现三处收尾加固：配置租约的异步释放不可被取消打断（与同步路径同一防护）、爬虫逐实例收尾异常不再阻断租约归还、刮削协程退出时的连接归还恒执行；并已用取消风暴/收尾窗口打断两类回归测试锁定
- **议题 #102 软件主界面三项 UI 交互修复**：①最小化主窗后操作 Emby 演员管理器，任意操作都把主窗拉回前台——根因 `eventFilter` 的 `ApplicationActivate` 分支对隐藏/最小化主窗无条件 `show()`，应用激活（点对话框、切任务）即触发。现最小化时维持状态不动（交给用户手动还原），仅非最小化的隐藏态保留原 show 行为；②主界面封面区（poster/thumb 图片框 + 尺寸文字）最大化后停留设计 220px 高度、不随窗口放大——绝对定位布局未把封面区纳入横向同步。现按设计基准宽 820 对 4 个封面 label 做横向等比缩放（宽高与 x 同 scale，y 不变，图片 `setScaledContents` 不变形）；③左侧状态提示浮框（正常模式/actor.json/MDCx 版本/点状数字）最大化后紧贴窗底「太靠下」——贴底公式无预留。现贴底预留 40px，状态区上移一行
- **Linux 构建支持**：新增 Ubuntu x86_64 手动构建与正式 Release 产物，使用锁定依赖和 PyQt6 所需系统库；Linux 打包跳过 macOS 专用 `.icns` 图标参数
- **MDCx 拦截自动二次启动（放行手工多开）**：检测网络/外部 CF 适配层路径曾触发「再 exec main.py」致双开主界面，任务管理器出现第二个 MDCx 进程、屏幕冒出两个一样的主窗口。现用环境变量标记 `MDCX_IS_FIRST_INSTANCE` 精确区分「自动二次启动」与「手工多开」：首次启动设置标记，自动 exec 继承的进程检测到标记即拦截退出；用户手工双击/命令行新开（全新进程树）不受影响，可自由多开刮削
- **议题 #106-① 带 DMM `z` 尾缀的番号在 javdb/javbus 搜不到（IBW 系列等）**：DMM 图床部分番号带 `z` 尾缀（如 `IBW-786z`，站内收录为 `IBW-786`），刮削直接拿带尾缀的番号搜索，javdb/javdb_app/javdb_api 一律「未收录该番号」、javbus 直链与搜索兜底都匹配不上。现提取番号时即剥掉 `z` 尾缀统一按基础番号（`IBW-786`）搜索与命名落库；搜索/匹配也一并接受去尾缀基础番号（`number_search_variants`、`match_number`、javbus `is_match`），兼顾手输或站内仍带尾缀的场景
- **议题 #106-② 剧照 20 张只保留 4 张（DMM 占位图字节阈值误杀真图）**：某标题（IBW-786z）实际有 20 张有效剧照，程序只下了 4 张（6/18/19/20）。根因 DMM 图片校验按体积把 `<4096B` 一律当占位图丢弃，而剧照探测缩略图（120x90）真实体积只有 2.8–4.5KB，16 张真图被误杀。占位图其实靠最终 URL 跳转 `now_printing` 就能识别；现把字节下限降到 2048，只兜住无跳转的垃圾响应（实测占位图探测后约 1.5KB、真图最小 2.8KB，取两者之间），并附小体积真图通过、垃圾响应被拒的回归测试
- **议题 #103 Jellyfin 12 演员列表回归「连接成功 0 个」（20260905 起）**：260823 版 Jellyfin 12 能取到 4144 个演员，20260905 起「查看信息/获取演员列表」一律显示 0 个。根因是 291da0b2 把 Jellyfin 分支的演员列表端点从 `/Persons` 切到 `/Items?includeItemTypes=Person`——Jellyfin 12 的 `/Items` 是通用 `BaseItemRepository` 路径，`personTypes` 只在配合 `Person`/`PersonIds` 时才参与过滤，且 Person 实体 `SupportsAncestors=false`、`TopParentId` 不指向任何 library，非递归 + 顶层库访问过滤下被静默滤空，恒返回 0 条；而 `/Persons` 走独立 `PeopleRepository` 直查 `Peoples` 表、`personTypes` 独立生效、不依赖层级，全量返回。现回退为 `/Persons`（保留 41fb16d 的完整 `MediaBrowser` 鉴权头以避免 401/超时）；`emby_actor_image._get_emby_actor_list` 与 `emby_actor_manager.get_emby_actor_list` 的 Jellyfin 分支同步改回 `/Persons`，并更新端点断言测试
- **议题 #106-③ 剧照改「有就下、没有就没有」，不再按 120x90 缩略图字节误杀**：前一条只是把 4096 降到 2048，本质仍是「按缩略图字节猜占位图」，对更小的真图（<2048B）仍会误杀。现剧照链路彻底不探字节：`_sanitize_extrafanart_urls` 只做 `pics.dmm.co.jp → awsimgsrc.dmm.co.jp/pics_dig` 无水印域名升级（全量、不抽检、不预下载），失效图（aws 404）交给下载层自然识别；`download_extrafanart_task` 在 AWS 无水印下载失败时回退 `pics.dmm.co.jp` 带水印原图再试一次（新增 `dmm_aws_to_pics_fallback_url` 反向映射，mono 老片补 `/adult/` 前缀）。不引入像素门槛——剧照原图 DMM 给多大就是多大，16 张缺图补回。公共层 2048 字节兜底保留，覆盖封面等其他 DMM 链路
