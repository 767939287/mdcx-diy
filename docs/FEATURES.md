...
- **演员库维护工具**：直接操作 `actor_database.xlsx`，一键补全中文名、LibreDMM 链接、别名（仅补缺别名的行）、剔除男演员、校验 tmdbid 有效性、更新 nfo tmdbid、打开数据库查看编辑（复用当前配置的 TMDB API），并发请求日志实时显示
- **网络连通性检查**：一键测试各网站可不可达
