...
- **反向查询**：已知中文名找日文名，或反过来
- **演员库维护工具**（工具页）：直接操作 `actor_database.xlsx`，三个独立按钮——补全中文名（按已有 TMDB ID 补中英繁体翻译）、补全 LibreDMM 链接（补信息链接）、同步别名（用 TMDB 最新 also_known_as 刷新 keyword 列），无需输入演员名单或选择 nfo 目录
...
- **演员库维护工具**：直接操作 `actor_database.xlsx`，一键补全中文名、LibreDMM 链接、同步别名（分别针对缺翻译/缺链接/别名过期的演员，复用当前配置的 TMDB API）
- **网络连通性检查**：一键测试各网站可不可达
